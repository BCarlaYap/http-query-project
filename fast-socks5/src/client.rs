    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    pub async fn use_stream(
        ...
        // Request Lifecycle
        println!("INFO    Requesting headers `{:?}`...", &stream.target_addr);
    ...
    async fn send_version_and_methods(
        ...
        println!(
"DEBUG  Send version and method len [{}, {}]",
        ...
        println!("DEBUG  client auth methods supported: {:?}", &auth);
        self.socket
            .write(&auth)
            .await
...
"DEBUG  Socks version ({version}), method chosen: {method}.",
        ...
        match method {
            consts::SOCKS5_AUTH_METHOD_NONE => println!("INFO    No auth will be used"),
    ...
    async fn use_password_auth(&mut self, methods: Vec<AuthenticationMethod>) -> Result<()> {
        println!("INFO    Password will be used");
...
"DEBUG  Auth: [version: {version}, is_success: {is_success}]",
    ...
    async fn request_header(&mut self) -> Result<()> {
        ...
        match self.target_addr {
            ...
            TargetAddr::Domain(ref domain, port) => {
                println!("DEBUG  TargetAddr::Domain");
        ...
        println!("DEBUG  Bytes long version: {:?}", &packet[..]);
        println!("DEBUG  Bytes shorted version: {:?}", &packet[..padding]);
        println!("DEBUG  Padding: {}", &padding);
    ...
    async fn read_request_reply(&mut self) -> Result<()> {
        ...
        println!(
"DEBUG  Reply received: [version: {version}, reply: {reply}, rsv: {rsv}, address_type: {address_type}]",
            version = version,
            reply = reply,
            rsv = rsv,
        ...
        println!("INFO    Remote server connected to {}.", address);
...
impl Socks5Stream<TcpStream> {
        ...
        T: ToSocketAddrs,
    {
        ...
        println!("INFO    Connected @ {}", &socket.peer_addr()?);
