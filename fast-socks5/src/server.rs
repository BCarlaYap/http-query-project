...
impl<T: AsyncRead + AsyncWrite + Unpin> Socks5Socket<T> {
    ...
    async fn get_methods(&mut self) -> Result<Vec<u8>> {
        ...
        println!("DEBUG  methods supported sent by the client: {:?}", &methods);
    ...
    async fn can_accept_method(&mut self, client_methods: Vec<u8>) -> Result<()> {
        ...
        if !client_methods.contains(&method_supported) {
            println!("DEBUG  Don't support this auth method, reply with (0xff)");
            self.inner
                .write(&[
    ...
    async fn authenticate(&mut self) -> Result<(String, String)> {
        ...
        println!("DEBUG  username bytes: {:?}", &username);

        let [pass_len] = read_exact!(self.inner, [0u8; 1]).context("Can't read pass len")?;
        println!("DEBUG  Auth: [pass len: {len}]", len = pass_len,);
        ...
        println!("DEBUG  password bytes: {:?}", &password);
        ...
        println!("INFO  User `{}` logged successfully.", username);
    ...
    async fn request(&mut self) -> Result<()> {
        ...
        if self.config.dns_resolve {
            ...
            println!("DEBUG  Domain won't be resolved because `dns_resolve`'s config has been turned off.")
        }
    ...
    async fn reply(&mut self, error: &ReplyError) -> Result<()> {
        ...
        println!("DEBUG  reply error to be written: {:?}", &reply);
    ...
    async fn read_command(&mut self) -> Result<()> {
        ...
        println!("DEBUG  Request target is {}", self.target_addr.as_ref().unwrap());
    ...
    async fn execute_command(&mut self) -> Result<()> {
        ...
        println!("DEBUG  Connected to remote destination");
    ...
    O: AsyncRead + AsyncWrite + Unpin,
{
    ...
    //         println!("INFO  local closed -> remote target ({} bytes consumed)", data)
    //     }
    //     Either::Left((Err(err), _)) => {
    //         error!("local closed -> remote target with error {:?}", err,)
    //     }
    //     Either::Right((Ok(data), _)) => {
    //         println!("INFO  local <- remote target closed ({} bytes consumed)", data)
    //     }
    ...
    let (inb_bytes, outb_bytes) = futures::future::join(inbound_to_outbound, outbound_to_inbound).await;

    if let Err(e) = &inb_bytes {
        println!("INBOUND ERROR: {:?}", e.to_string());
       // return Err(Error::new(e.kind(),format!("{}",e.to_string())));
    };


    if let Err(e) = &outb_bytes {
        println!("OUTBOUND ERROR: {:?}", e.to_string());
       // return Err(Error::new(e.kind(), format!("{}",e.to_string())));
    }

    let inb = inb_bytes.unwrap();
    let outb = outb_bytes.unwrap();
    println!("INFO  INBOUND: {}  OUTBOUND: {}",&inb, &outb);
    let mut x = inb.to_ne_bytes().to_vec();
    let y = outb.to_ne_bytes();
    ...
    wi.write_all(x.as_slice()).await
        .context("EXTRA INBOUND WRITE???")?;
